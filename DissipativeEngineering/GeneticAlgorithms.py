...
class StandardGeneticAlgorithm:
    ...
    def __init__(self, num_params,  population_size):


        "define parameters"
        self.N_pop = population_size
        self.N_var = num_params
        self.decimals = 2
    "Initialize a first population"
    def initial_population(self):
        return np.round(np.random.uniform(-1,1, (self.N_pop, self.N_var)), decimals = self.decimals)
    ...
    def pairing(self, survivors, survivor_fitness):
        ...
        for i, ma in enumerate(ma_indices):
            ...
            inter_prob_dist = inter_prob_dist/(np.sum(inter_prob_dist))
...
#                                    size=int(self.N_pop/4), p=prob_dist, replace=False)
    ...
    def mating_procedure(self, ma, da):
        ...
        return np.round(offspring_array, decimals=self.decimals)
    ...
    def mutate(self, population, rate=0.3):
        ...
        return np.round(mutated_population, decimals=self.decimals)
